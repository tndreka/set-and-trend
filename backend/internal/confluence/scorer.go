...
func (cs *ConfluenceScorer) CalculateConfluence(
	...
	if len(h2Candles) >= 20 {
		score.H2Psych = cs.calculatePsychLevelScore(h2Candles) * 0.028
	}
...
func (cs *ConfluenceScorer) detectBestPattern(candles []patterns.Candle, symbol string) *patterns.TradeSignal {
	...
	dt := patterns.DetectDoubleTop(candles, 50)
	if dt != nil {
		ctx := patterns.DetermineMarketContext(candles)
	...
	db := patterns.DetectDoubleBottom(candles, 50)
	if db != nil {
		ctx := patterns.DetermineMarketContext(candles)
...
func (cs *ConfluenceScorer) calculatePsychLevelScore(candles []patterns.Candle) float64 {
	if len(candles) < 1 {
		return 0.0
	}
	...
	// Round to nearest 100 pips (0.0100 for EURUSD)
	rounded := math.Round(price/0.01) * 0.01
	
	// Within 5 pips of psych level
	if math.Abs(price-rounded) <= 5*pipSize {
		return 1.0
	}
